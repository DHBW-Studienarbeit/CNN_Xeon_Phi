...
void network_forward(NeuronalNetwork_p network, DataSupplier_p input_supply)
{
    ...
    for current in net._layers:
        if current.__class__.__name__ == "MaxPoolingLayer":
            ...
            if first==True:
                cog.out(", datasupply_get_input(input_supply)")
            cog.outl(");")
        else:
            cog.out(current.get_C_forwardname(first) + "(network->layer_" \
    ...
    layer_conv_first_forward(network->layer_0, net_activations, datasupply_get_input(input_supply));
...
void network_backward(NeuronalNetwork_p network, DataSupplier_p input_supply)
{
    ...
    layer_conv_first_backward(network->layer_0, net_activations, datasupply_get_input(input_supply), net_activations_errors, net_weights_f_errors);
    // [[[end]]]
}
