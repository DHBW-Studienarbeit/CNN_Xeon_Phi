...
Float_t network_get_cost(NeuronalNetwork_p network, Float_p labels)
{
    ...
    cog.out(", net_activations + network->layer_")
    cog.out(str(len(net._layers)-1))
    cog.out(".output_activation_offset, labels, shared_tmp_floats);")
    ]]] */
    return get_cost(10, 10, net_activations + network->layer_4.output_activation_offset, labels, shared_tmp_floats);
    // [[[end]]]
}
...
void network_derive_cost(NeuronalNetwork_p network, Float_p labels)
{
    ...
    cog.out(", net_activations + network->layer_")
    cog.out(str(len(net._layers)-1))
    cog.out(".output_activation_offset, labels, net_activations_errors + network->layer_")
    cog.out(str(len(net._layers)-1))
    cog.out(".output_activation_offset);")
    ]]] */
    get_cost_derivatives(100, net_activations + network->layer_4.output_activation_offset, labels, net_activations_errors + network->layer_4.output_activation_offset);
    // [[[end]]]
}
